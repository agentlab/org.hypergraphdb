...
public class SubgraphManager
{
    ...
    public static void dumpGraphToFile(File file, StorageGraph graph, boolean outputByteBuffers)
    {
        try
        {
            ...
            TreeMap<HGPersistentHandle, Object> sorted = new TreeMap<>();
     ...
     *
     */
    public static void store(final StorageGraph subgraph, final HGStore store,
                             final Map<HGPersistentHandle, HGPersistentHandle> substitute)
    {
        ...
        store.getTransactionManager().transact(new Callable<Object>()
        {
            ...
            public Object call() {
                ...
                {
                    if (substitute.containsKey(item.getFirst()))
                    {
                        continue;
                    }
     ...
     * @return
     */
    public static Object getTransferObjectRepresentation(HyperGraph graph, HGHandle atomHandle, Object atom,
                                                         HGHandle typeHandle)
    {
        if (typeHandle == null)
        {
        ...
        try
        {
            ...
            return Json.object("storage-graph", encodeSubgraph(sgraph),
                "type-handle", typeHandle, "type-classes", types);
        }
        finally
        {
...
/*        	if (graph == null)
    ...
    public static Object readAtom(HGHandle handle, HyperGraph graph, RefResolver<HGHandle, HGAtomType> typeResolver,
                                  StorageGraph subgraph)
    {
        ...
        try
        {
            ...
            object = type.make(layout[1], new ReadyRef<>(targetSet), null);
        }
        finally
        {
            graph.getStore().detachOverlayGraph();
     ...
     */
    private static int translateBatch(HyperGraph graph, Set<HGHandle> batch,
...
//            }
            ...
            HGHandle existing = atomFinder == null ? null :
                atomFinder.eval(new Pair<>(atom, object));
            if (existing != null)
            {
                                                        ...
                                                        HGHandle> atomFinder)
    {
    	...
    	// Boris
    	//
        final Map<HGHandle, HGHandle> substitutes = new HashMap<>();
        final Set<HGHandle> batch = new HashSet<>();
        final Map<HGHandle, HGHandle> currentChanges = new HashMap<>();
        ...
        throws ClassNotFoundException
    {
        ...
        Map<HGHandle, Object> objects = new HashMap<>();
    ...
    {
        Set<HGHandle> result = new HashSet<>();
        final Set<HGPersistentHandle> batch = new HashSet<>();
        for (HGPersistentHandle theRoot : subgraph.getRoots())
        {
    ...
    private static class AtomFilteringSubgraph implements StorageGraph
    {
        ...
        AtomFilteringSubgraph(HyperGraph graph, StorageGraph wrapped)
        {
            ...
            indexByValue = graph.getStore().getIndex(HyperGraph.VALUES_INDEX_NAME,
                BAtoHandle.getInstance(graph.getHandleFactory()), BAtoHandle.getInstance(graph.getHandleFactory()),
                                          null,
                                          null,
                                          true);
        ...
        {
            return new FilterIterator<>(wrapped.iterator(),
