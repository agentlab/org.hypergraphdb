...
public /*final*/ class HyperGraph
    implements HyperNode, AutoCloseable
{
    public static final HGHandle [] EMPTY_HANDLE_SET = new HGHandle[0];
    public static final LazyRef<HGHandle[]> EMPTY_HANDLE_SET_REF = new ReadyRef<>(new HGHandle[0]);
...
//    public static final String TYPES_CONFIG_FILE = "/org/hypergraphdb/types";
    ...
    private synchronized void open()
    {
    	...
    	try
    	{
	        ...
	        //
            indexByType = store.getIndex(TYPES_INDEX_NAME, BAtoHandle.getInstance(this.getHandleFactory()),
                BAtoHandle.getInstance(this.getHandleFactory()),
	                                     null,
	                                     null,
                true);
            indexByValue = store.getIndex(VALUES_INDEX_NAME, BAtoHandle.getInstance(this.getHandleFactory()),
                BAtoHandle.getInstance(this.getHandleFactory()),
	                                      null,
	                                      null,
                true);
...
//	        typeSystem.getJavaTypeFactory().initNonDefaultMappers();
    	...
    	catch (Throwable t)
    	{
    		if (store != null)
            {
                try { store.close(); } catch (Throwable t1) { }
            }
...
//}});
    ...
    private boolean removeTransaction(final HGHandle handle, final boolean keepIncidentLinks)
    {
        ...
        Set<HGPersistentHandle> inRemoval =
            TxAttribute.getSet(getTransactionManager(), TxAttribute.IN_REMOVAL, HashSet.class);
        ...
        try
        {
	        ...
	        if (atom instanceof HGAtomType)
            {
	        	...
	        	try
	        	{
                    ...
                    {
                        removeTransaction(instances.next(), keepIncidentLinks);
                    }
	        	}
	        	finally
	        	{
            ...
            if (keepIncidentLinks)
	        {
	            ...
	            try { while (rsInc.hasNext())
                {
                    targetRemoved(rsInc.next(), pHandle);
                } }
                finally
                {
                    rsInc.close();
...
//	        			removeTransaction(h, false);
     ...
     */
    public void define(final HGHandle atomHandle, final HGHandle typeHandle, final HGHandle valueHandle,
    				   ...
    				   final int flags)
    {
        ...
        public Object call() {
	    	...
	    	if (outgoingSet != null)
	    	{
		    	...
		    	linkRef = new ReadyRef<>(targets);
    		}
                HGAtomType type = get(typeHandle);
                idx_manager.maybeIndex(layout[0], type,
     ...
     */
    @Override
    public void define(final HGHandle atomHandle, final HGHandle typeHandle, final Object instance,
                       final int flags)
    {
        getTransactionManager().ensureTransaction(new Callable<Object>()
        {
     ...
     */
    private HGLiveHandle addLink(final Object payload, final HGHandle typeHandle, final HGLink outgoingSet,
    							 final byte flags)
    {
        return getTransactionManager().ensureTransaction(new Callable<HGLiveHandle>()
        { @Override
     ...
     * @return
     */
    private Pair<HGLiveHandle, Object> loadAtom(final HGPersistentHandle persistentHandle,
        final HGLiveHandle liveHandle)
    {
        ...
        public Pair<HGLiveHandle, Object> call() {
	        ...
	        if (typeHandle.equals(typeSystem.getTop()))
	        {
	        	...
	        	return new Pair<>(result, result.getRef());
	        }
	        ...
	        try
	        {
                ...
                else
		        {
		            ...
		            for (int i = 2; i < link.length; i++)
		            {
                        ...
                        instance = type.make(valueHandle, new ReadyRef<>(targets),
		            					 isref);
...
//    	TypeUtils.initiateAtomConstruction(HyperGraph.this, layout[1]);
        Object result = type.make(layout[1], new ReadyRef<HGHandle[]>(targetSet),
...
//    	TypeUtils.atomConstructionComplete(HyperGraph.this, layout[1]);
     ...
     */
    private void replaceInternal(final HGLiveHandle lHandle, final HGPersistentHandle pHandle, final Object atom,
     ...
     * @param typeHandle The type of the new value
     */
    private Object replaceTransaction(HGLiveHandle lHandle, final HGPersistentHandle pHandle, final Object atom,
    							 	final HGHandle typeHandle)
    {
        ...
        //
	    	if (oldValue instanceof HGAtomType)
	    	{
	    		...
	    		try
	    		{
		    		...
		    		while (rs.hasNext())
                    {
                        morph(rs.next(), oldTypeValue, newTypeValue);
                    }
	    		}
	    		finally
	    		{
	    	...
	    	if (!oldTypeHandle.equals(typeHandle))
        {
            ...
            // sets where it no longer belongs.
	    		HashSet<HGPersistentHandle> newTargets = new HashSet<>();
    ...
    private void initAtomManagement()
    {
    	...
    	// Init HGStats atom.
    	//
        getTransactionManager().transact(new Callable<Object>() { @Override
        public Object call() {
...
//    			);
    ...
    private void loadListeners()
    {
    	...
    	try
    	{
    		...
    		while (rs.hasNext())
    		{
    			HGListenerAtom listenerAtom = (HGListenerAtom)get(rs.next());
    			Class<?> eventClass;
    			Class<?> listenerClass;
    			try
    			{
